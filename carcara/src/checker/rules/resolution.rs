...
use crate::{ast::*, checker::Elaborator, resolution::*, utils::DedupIterator};
...
pub fn elaborate_resolution(
    ...
    let mut premise_clauses: Vec<_> = premises.iter().map(|p| p.clause).collect();

    let ResolutionTrace { not_not_added, pivot_trace } =
        greedy_resolution(conclusion, &premise_clauses, pool, true).or_else(|_| {
            premises.reverse();
            premise_clauses.reverse();
            greedy_resolution(conclusion, &premise_clauses, pool, true)
        })?;
